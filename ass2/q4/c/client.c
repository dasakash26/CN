...
void sender_piggybacked(int sock, struct sockaddr_in *addr) {
    ...
    while (base < total_frames) {
        ...
        // Wait for response with piggybacked ACK
        struct timeval tv;
        tv.tv_sec = TIMEOUT;
        tv.tv_usec = 0;
        setsockopt(sock, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv));

        // Clear receive buffer before receiving
        ...
        if (len > 0) {
            if (recv_frame.ack != -1) {
                ...
                // Display the piggybacked data
                if (recv_frame.seq_num != -1) {
                    printf("Piggybacked frame %d received with data: %s\n", 
                           recv_frame.seq_num, recv_frame.data);
                } else {
                    printf("ACK only frame received with message: %s\n", recv_frame.data);
                }

                // Mark acknowledged frames
                ack_received[recv_frame.ack] = 1;
...
int main() {
    ...
    serv_addr.sin_addr.s_addr = inet_addr("100.69.69.69");  // Make sure this is the correct IP

    sender_piggybacked(sock, &serv_addr);
